...
manifest={'app':'Notesy','version':version,'status':'public-beta','uuid':json.loads((root/'package.json').read_text())['pebble']['uuid'],'license':'MIT','artifacts':[{'name':p.name,'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts]}
