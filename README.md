...
The Mac connector’s Notes folder setting supports a new name or an existing subfolder inside the vault, including nested paths. `Pebble` is the default. Only immediate Markdown children of the selected folder are listed. Changing destinations leaves existing notes in place; test and save phone settings again. Pending deliveries remain bound to their original folder. Trash is inside the selected folder as `.trash`.
