...
Notesy 1.4.6 is released alongside
...
[download the 1.4.6 public beta](https://github.com/GeezusChrotch/notesy/releases/tag/v1.4.6).
See [release status](docs/RELEASE_STATUS.md) for validation details.

Browse an Obsidian vault, read notes and capture dictation on Pebble Time and Time 2. Version 1.4.6 · public beta.
...
The main page shows pins first, then folders, then root notes. In 1.4.6, Actions → Sort notes and phone Vault settings offer name, modified date, created date or scoped tag browsing. Return to top is available in Actions and as a configurable shortcut. See [browsing and sorting](docs/USER_GUIDE.md#browsing-sorting-and-returning-to-the-top-120); install the matching Connector service before this watch build.
...
Notesy 1.4.6 includes selectable note links and richer Markdown. See
[note links and Markdown](docs/MARKDOWN_AND_LINKS.md) for behavior and supported syntax.
Requires Organik Apps Pebble Connector 0.8.0 or later.
...
Pebble → Notesy → Settings, test, and save. Install `dist/Notesy-1.4.6.pbw` on the watch.
...
Use `build/StoneNotes.pbw` for debugging and `dist/Notesy-1.4.6.pbw` for distribution; the latter
...
service. Normal watch installations must use `dist/Notesy-1.4.6.pbw`.
...
The release passes automated tests and Basalt/Emery builds. Native and earlier emulator checks cover scrolling, links, tasks, formatting, Double Back and menu startup. This exact 1.4.6 package was installed successfully; visual acceptance of PDF previews and image modes remains pending. Fresh Mac/phone setup and end-to-end hardware acceptance remain pending. See [release status](docs/RELEASE_STATUS.md).
