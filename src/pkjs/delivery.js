...
function Delivery(storage, send, notify) {
  ...
  this.enqueue = function(id, text, config) {
    if (state.done.indexOf(id) >= 0) { notify('saved', id); return; }
    var existing=state.pending.filter(function(p) { return p.id === id; })[0];
    if (existing) { if(existing.text!==text)throw Error('Draft ID conflict. Your original pending note was preserved.');notify('queued', id); return; }
    ...
    commit({pending:state.pending.concat([{id:id,text:text,vaultId:config.vaultId,origin:config.gatewayURL}]),done:state.done});
    notify('queued', id);
  };
  this.pump = function(config) {
    ...
    busy = true;
    send(config, {requestId:note.id,text:note.text,vaultId:note.vaultId}, function(error, value) {
      busy = false;
