...
function settingsMessage(){return Themes.message(config,enhanced());}
...
Pebble.addEventListener('ready',function(){send(settingsMessage());pump();});
...
Pebble.addEventListener('webviewclosed',function(event){
  ...
  try{var c=JSON.parse(decodeURIComponent(event.response));if(!valid(c))throw Error('Invalid pairing details.');localStorage.setItem('stonenotes.config',JSON.stringify(c));config=c;send(settingsMessage());pump();}catch(e){error('Settings were not saved. Check the pairing details.');}
});
