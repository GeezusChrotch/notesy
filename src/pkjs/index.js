...
function settingsMessage(){var m=Themes.message(config,enhanced());m.BUTTONS=Buttons.normalize(config&&config.buttons).join(',');m.API=config&&config.browserId?2:1;m.VAULT_ID=config&&config.browserId||'';m.FOLDER_ID=config&&config.root||'';return m;}
...
Pebble.addEventListener('appmessage',function(event){
  ...
  if(m.COMMAND===1){
    ...
    request(config,'GET',(m.API===2?'/v2/notes/':'/v1/notes/')+encodeURIComponent(m.NOTE_ID)+'?page='+(m.PAGE||0),null,function(e,value){
      if(seq!==activeRequest)return;if(e){error(e.message,seq);return;}
