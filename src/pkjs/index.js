...
function imageMode(value){return ['natural','high-contrast','original'].indexOf(value)>=0?value:'natural';}
...
Pebble.addEventListener('appmessage',function(event){
  ...
  if(m.COMMAND===9){
    request(config,'GET','/v3/notes/'+encodeURIComponent(m.NOTE_ID)+'/image?index='+m.INDEX+'&revision='+encodeURIComponent(m.REVISION)+'&width='+m.WIDTH+'&height='+m.HEIGHT+'&mode='+imageMode(config&&config.imageMode),null,function(e,v){
...
function configurationURL(){
  ...
  html=html.replace('</script>', 'document.getElementById("image-mode").value='+JSON.stringify(imageMode(config&&config.imageMode))+';</script>');
