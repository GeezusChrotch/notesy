...
static NoteTrail s_note_trail[12];static int s_note_depth,s_link_scroll=-1;
static char s_link_target[65];static bool s_link_back;
...
static bool s_stitch,s_capture_choices,s_sort_choices,s_tag_picker,s_restoring_list;
static uint8_t s_sort;static char s_tag[65];
static AppTimer *s_browse_timer;static int s_browse_direction;
static void browse_selection(MenuLayer *menu,MenuIndex next,MenuIndex previous,void *context);
...
static MenuLayer *marquee_menu(void){
  ...
  if(top==s_reader&&s_rich)return NULL;
...
static void send_command(int command, const char *id, int page, const char *text) {
  ...
  if (app_message_outbox_begin(&iter) != APP_MSG_OK) { cancel_link();stop_stitch();s_loading=false;set_status("Phone busy · please retry"); return; }
  ...
  if(command==1||command==10)dict_write_cstring(iter,MESSAGE_KEY_FOLDER_ID,s_folder);
  if(command==1){dict_write_uint8(iter,MESSAGE_KEY_SORT,s_sort);dict_write_cstring(iter,MESSAGE_KEY_TAG,s_tag);}
  if(command==1||command==7||command==10)dict_write_cstring(iter,MESSAGE_KEY_SNAPSHOT,s_snapshot);
  ...
  if (app_message_outbox_send() != APP_MSG_OK) { cancel_link();stop_stitch();s_loading=false;set_status("Phone unavailable · draft kept"); return; }
...
static void load_notes(int offset) {
  ...
  set_status(s_tag_picker?"Loading tags…":s_search?"Searching vault…":"Loading folder…"); send_command(s_tag_picker?10:s_search?7:1, NULL, offset, s_search?s_query:NULL);
}
#if defined(PBL_PLATFORM_EMERY)
...
static void draw_row(GContext *ctx, const Layer *cell, MenuIndex *index, void *context) {
  ...
  if(row==0){title=s_tag_picker?"Sort notes":s_search?"Search again":s_pending?"Retry draft":"New note";subtitle=s_status;}
  else if(row<=s_count){
    static char date[40],label[112];Note *n=&s_notes[row-1];title=n->folder?n->title:note_title(n->title,date,sizeof(date));
    snprintf(label,sizeof(label),"%s%s%s",n->pinned?"Pinned / ":"",(s_search||s_tag_picker)?n->location:n->folder?"Folder":(date[0]?date:"Note"),"");subtitle=label;
  }else title="Loading…";
...
static void move_selection(bool down){
  ...
  if(down&&row==s_count&&s_offset+s_count<s_total){s_restore_row=1;load_notes(s_offset+s_count-1);return;}
  if(!down&&row<=1&&s_offset>0){s_restore_row=MAX_NOTES;load_notes((s_offset>MAX_NOTES-1?s_offset-(MAX_NOTES-1):0));return;}
...
// Native touch menus can change focus without going through our button handler.
// Keep the boundary item on the next page, so paging never skips that item.
static void browse_boundary(void *context){
  s_browse_timer=NULL;if(s_loading||s_restoring_list||s_body||!s_menu)return;
  int row=menu_layer_get_selected_index(s_menu).row;
  if(s_browse_direction>0&&row==s_count&&s_count>1&&s_offset+s_count<s_total){s_restore_row=1;load_notes(s_offset+s_count-1);}
  else if(s_browse_direction<0&&row<=1&&s_offset>0){int offset=s_offset>14?s_offset-14:0;s_restore_row=s_offset-offset+1;load_notes(offset);}
}
static void browse_selection(MenuLayer *menu,MenuIndex next,MenuIndex previous,void *context){
  marquee_reset();if(s_browse_timer){app_timer_cancel(s_browse_timer);s_browse_timer=NULL;}
  if(s_restoring_list||s_loading||menu!=s_menu)return;
  if(next.row==s_count&&next.row>previous.row&&s_offset+s_count<s_total)s_browse_direction=1;
  else if(next.row<=1&&next.row<previous.row&&s_offset>0)s_browse_direction=-1;
  else return;
  s_browse_timer=app_timer_register(150,browse_boundary,NULL);
}
static void press(ClickRecognizerRef recognizer,void *context){
  ...
  if(button!=1){if(s_body)scroll_note(button==2);else move_selection(button==2);return;}
  ...
  else {if(s_rich)rich_select();else perform(s_body?5:8);}
}
static void long_press(ClickRecognizerRef recognizer,void *context){
  if(s_stitch){set_status("Stitch in progress · Back to finish");return;}
  int button=click_recognizer_get_button_id(recognizer)-BUTTON_ID_UP;
  if(button>=0&&button<=2){int action=s_buttons[(s_body?9:3)+button];if(action)perform(action==8?11:action==9?12:action==10?13:action==11?14:action==12?15:action);else if(button==1){if(s_rich)rich_select();else perform(s_body?5:8);}else if(s_body)scroll_note(button==2);else move_selection(button==2);}
}
static void back_click(ClickRecognizerRef recognizer,void *context){
  ...
  if(s_body){
    if(s_link_target[0]){cancel_link();++s_request;clear_timeout();s_loading=false;set_status("Link cancelled");return;}
    if(s_note_depth){NoteTrail *trail=&s_note_trail[--s_note_depth];snprintf(s_link_target,sizeof(s_link_target),"%s",trail->id);s_link_back=true;s_rich_restore=trail->row;s_link_scroll=trail->scroll;s_loading=true;image_clear();send_command(2,s_link_target,trail->page,NULL);return;}
    window_stack_pop(true);if(!s_snapshot[0])refresh_list();return;
  }
  if(s_tag_picker||(!s_search&&s_sort==3)){
    bool was_picker=s_tag_picker;s_tag_picker=!was_picker;if(was_picker)s_sort=0;s_tag[0]=0;s_snapshot[0]=0;s_count=0;s_offset=0;s_restore_row=1;load_notes(0);return;
  }
  if(s_search){
    s_search=false;s_query[0]=0;Location *loc=&s_before_search;
    memcpy(s_folder,loc->id,sizeof(s_folder));memcpy(s_folder_title,loc->title,sizeof(s_folder_title));s_snapshot[0]=0;s_restore_row=loc->row;
...
static void notesy_menu_selected(MenuLayer *menu,MenuIndex *index,void *context){
  if(s_loading||s_stitch||menu!=s_menu||index->section||index->row>s_count)return;
  menu_layer_set_selected_index(menu,*index,MenuRowAlignCenter,false);
  perform(8);
}
#if defined(PBL_TOUCH)
// Touch navigation bypasses customizable button actions, including Delete.
static void notesy_touch_main_swipe(const Recognizer *recognizer, RecognizerEvent event) {
...
static void notesy_touch_main_tap(const Recognizer *recognizer, RecognizerEvent event) {
  ...
  GPoint origin=layer_convert_point_to_screen(menu_layer_get_layer(s_menu),GPointZero);
  GRect bounds=layer_get_bounds(menu_layer_get_layer(s_menu));
  if(point.x<origin.x||point.x>=origin.x+bounds.size.w||point.y<origin.y||point.y>=origin.y+bounds.size.h)return;
  int y=point.y-origin.y-scroll_layer_get_content_offset(menu_layer_get_scroll_layer(s_menu)).y;
  for(int row=0;row<=s_count;row++){
    MenuIndex index=MenuIndex(0,row);int height=row_height(s_menu,&index,NULL);
    if(y>=0&&y<height){
      MenuIndex selected=menu_layer_get_selected_index(s_menu);
      if(selected.section!=index.section||selected.row!=index.row)
        menu_layer_set_selected_index(s_menu,index,MenuRowAlignCenter,false);
      else notesy_menu_selected(s_menu,&index,NULL);
      return;
    }
    y-=height;
  }
...
static uint16_t action_rows(MenuLayer *menu,uint16_t section,void *context){return s_sort_choices?4:s_capture_choices?2:s_body?9:11;}
static int action_code(int row){static const int browsing[]={8,1,12,11,2,13,4,3,7,14,15},reading[]={2,13,11,4,3,1,7,14,15};return s_sort_choices?20+row:s_capture_choices?(row?12:1):s_body?reading[row]:browsing[row];}
static void action_draw(GContext *ctx,const Layer *cell,MenuIndex *index,void *context){
  ...
  const char *title=action==20?"Name (A–Z)":action==21?"Date modified":action==22?"Date created":action==23?"Tag":action==14?"Return to top":action==15?"Sort notes":action==11?"Dictate to search":action==8?"Open selected":action==1?"Quick Dictate":action==12?"Stitch":action==2?"Append: Quick Dictate":action==13?"Append: Stitch":action==4?(pinned?"Unpin":"Pin to main page"):action==3?"Delete note":"Refresh";
...
static void actions_load(Window *window){
  s_action_menu=organik_menu_create(layer_get_bounds(window_get_root_layer(window)));
...
static void rich_draw(GContext *ctx,const Layer *cell,MenuIndex *index,void *context){
  GRect bounds=layer_get_bounds(cell);bool selected=index->row==s_rich_active;
  graphics_context_set_fill_color(ctx,selected?s_highlight:s_background);graphics_fill_rect(ctx,bounds,0,GCornerNone);
  ...
  if(item->kind==1){
    ...
    graphics_draw_text(ctx,item->text,theme_title_font(),GRect(28,4,bounds.size.w-34,bounds.size.h-8),GTextOverflowModeWordWrap,GTextAlignmentLeft,NULL);
    ...
    graphics_draw_text(ctx,item->kind==3?"Open linked note >":"Link unavailable",fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(6,s_theme_size+10,bounds.size.w-12,22),GTextOverflowModeTrailingEllipsis,GTextAlignmentLeft,NULL);
  }else {graphics_context_set_stroke_color(ctx,selected?s_selection_text:s_foreground);markdown_layout(ctx,item,bounds.size.w-12,4);}
}
// Each block owns a regular clipped Layer. No MenuLayer cursor/cache is involved.
static bool rich_interactive(int row){return row>=0&&row<s_rich_count&&(s_rich_items[row].kind==1||s_rich_items[row].kind==3);}
static int rich_document_height(void){int height=0;for(int i=0;i<s_rich_count;i++){MenuIndex index=MenuIndex(0,i);height+=rich_height(s_document_view,&index,NULL);}return height;}
static int rich_scroll_max(void){int max=rich_document_height()-layer_get_bounds(s_document_view).size.h;return max>0?max:0;}
static void rich_visible(bool choose_link){
  if(s_loading||!s_document_view)return;
  int viewport=layer_get_bounds(s_document_view).size.h;
  int y=0,best=-1,distance=32767,image=-1,image_area=0;
  for(int row=0;row<s_rich_count;row++){
    MenuIndex index=MenuIndex(0,row);int height=rich_height(s_document_view,&index,NULL),top=y-s_rich_scroll,bottom=top+height;
    if(top<viewport&&bottom>0){
      if(rich_interactive(row)&&top>=0&&bottom<=viewport){int d=top+height/2-viewport/2;if(d<0)d=-d;if(d<distance){best=row;distance=d;}}
      if(s_rich_items[row].kind==2){int area=(bottom<viewport?bottom:viewport)-(top>0?top:0);if(area>image_area){image=row;image_area=area;}}
    }
    y+=height;
  }
  if(choose_link)s_rich_active=best;
  if(image<0&&s_image_error[0])image_clear();
  if(image>=0&&s_image_index!=s_rich_offset+image){
    if(s_image_loading){++s_request;clear_timeout();}
    image_clear();s_image_index=s_rich_offset+image;s_image_loading=true;send_command(9,s_current_id,0,NULL);
  }
}
static void rich_position(int position,bool choose_link){
  int max=rich_scroll_max();if(position<0)position=0;if(position>max)position=max;s_rich_scroll=position;
  scroll_layer_set_content_offset(s_document_scroll,GPoint(0,-position),s_rich_animate);
  rich_visible(choose_link);layer_mark_dirty(s_document_view);
}
static void rich_scroll_by(int delta){
  if(s_loading||!s_document_view)return;
  bool down=delta>0;int max=rich_scroll_max();
  if((down&&s_rich_scroll==max&&s_rich_offset+s_rich_count<s_rich_total)||(!down&&s_rich_scroll==0&&s_rich_offset>0)){
    s_rich_restore=down?0:14;s_rich_active=-1;s_loading=true;image_clear();send_command(2,s_current_id,s_rich_offset/15+(down?1:-1),NULL);return;
  }
  rich_position(s_rich_scroll+delta,true);
}
static void rich_move(bool down){s_rich_animate=true;rich_scroll_by(down?(s_theme_size/2+5):-(s_theme_size/2+5));s_rich_animate=false;}
static void rich_select(void){
  if(s_loading||!s_document_view){return;}int row=s_rich_active;if(!rich_interactive(row)){open_actions(NULL,NULL);return;}RichItem *item=&s_rich_items[row];
  if(item->kind==4){set_status(item->text);return;}
  if(item->kind==3){
    if(s_note_depth>=12){set_status("Back through links before opening more");return;}
    NoteTrail *trail=&s_note_trail[s_note_depth++];snprintf(trail->id,sizeof(trail->id),"%s",s_current_id);trail->page=s_page;trail->row=row;trail->scroll=s_rich_scroll;
    snprintf(s_link_target,sizeof(s_link_target),"%s",item->id);s_link_back=false;s_rich_restore=0;s_link_scroll=-1;s_loading=true;image_clear();set_status("Opening linked note…");send_command(2,s_link_target,0,NULL);return;
  }
...
#if defined(PBL_TOUCH)
static void notesy_rich_selected(Layer *menu,MenuIndex *index,void *context){
  if(s_loading||s_stitch||menu!=s_document_view||index->section||index->row>=s_rich_count)return;
  if(!rich_interactive(index->row))return;
  s_rich_active=index->row;rich_select();
}
static void notesy_reader_tap_at(GPoint point){
  if(!touch_service_is_enabled()||s_loading||s_stitch)return;
  if(!s_rich)return;
  if(!s_document_view)return;
  GPoint origin=layer_convert_point_to_screen(s_document_view,GPointZero);
  GRect bounds=layer_get_bounds(s_document_view);
  if(point.x<origin.x||point.x>=origin.x+bounds.size.w||point.y<origin.y||point.y>=origin.y+bounds.size.h)return;
  int y=point.y-origin.y-scroll_layer_get_content_offset(s_document_scroll).y;
  for(int row=0;row<s_rich_count;row++){
    MenuIndex index=MenuIndex(0,row);int height=rich_height(s_document_view,&index,NULL);
    if(y>=0&&y<height){
      if(!rich_interactive(row))return;
      if(s_rich_active!=row){s_rich_active=row;layer_mark_dirty(s_document_view);}
      else notesy_rich_selected(s_document_view,&index,NULL);
      return;
    }
    y-=height;
  }
}
static bool s_reader_touch_tracking,s_reader_touch_vertical;static GPoint s_reader_touch_start;static int s_reader_touch_max,s_reader_touch_last_y;
static void notesy_reader_touch(const TouchEvent *event,void *context){
  ...
  if(event->type==TouchEvent_Touchdown){s_reader_touch_tracking=true;s_reader_touch_start=GPoint(event->x,event->y);s_reader_touch_max=0;s_reader_touch_vertical=false;s_reader_touch_last_y=event->y;return;}
  ...
  if(s_rich&&(s_reader_touch_vertical||(ay>=10&&ay>ax*2))){
    s_reader_touch_vertical=true;int delta=s_reader_touch_last_y-event->y;s_reader_touch_last_y=event->y;
    if(delta)rich_scroll_by(delta);
    if(event->type==TouchEvent_Liftoff)s_reader_touch_tracking=false;
    return;
  }
  ...
  else if(ay>=28&&ay>ax*2){if(s_rich)rich_scroll_by(-dy);else scroll_note(dy<0);}
}

#endif
static void document_cell_draw(Layer *cell,GContext *ctx){
  int row=*(uint8_t*)layer_get_data(cell);MenuIndex index=MenuIndex(0,row);rich_draw(ctx,cell,&index,NULL);
}
static void document_reload(void){
  int y=0,width=layer_get_bounds(s_document_view).size.w;
  for(int row=0;row<15;row++){
    Layer *cell=s_document_cells[row];layer_set_hidden(cell,row>=s_rich_count);
    if(row<s_rich_count){MenuIndex index=MenuIndex(0,row);int height=rich_height(s_document_view,&index,NULL);layer_set_frame(cell,GRect(0,y,width,height));layer_set_bounds(cell,GRect(0,0,width,height));y+=height;}
  }
  scroll_layer_set_content_size(s_document_scroll,GSize(width,y));
}
static void document_destroy(void){
  for(int row=0;row<15;row++){if(s_document_cells[row])layer_destroy(s_document_cells[row]);s_document_cells[row]=NULL;}
  if(s_document_scroll){scroll_layer_destroy(s_document_scroll);}s_document_scroll=NULL;s_document_view=NULL;
}
...
static void open_selected(void){
  if(s_loading)return;
  if(s_tag_picker){Note *tag=selected_note();if(!tag){perform(15);return;}snprintf(s_tag,sizeof(s_tag),"%s",tag->id);s_tag_picker=false;s_snapshot[0]=0;s_count=0;s_offset=0;s_restore_row=1;load_notes(0);return;}
  ...
  s_note_depth=0;s_link_scroll=-1;s_link_target[0]=0;s_delete_id[0]=0;s_scroll_to_end=false;s_note_pinned=n->pinned;
...
static void return_to_top(void){
  if(s_body)window_stack_pop(true);
  ++s_request;clear_timeout();s_loading=false;s_search=false;
  s_snapshot[0]=0;s_count=0;s_offset=0;s_restore_row=1;load_notes(0);
}
static void perform(int action){
  ...
  if(s_stitch){set_status("Stitch in progress · Back to finish");return;}
  if(s_tag_picker&&(action==2||action==3||action==4||action==13)){set_status("Choose a note first");return;}
  if(action==11){s_tag_picker=false;start_search();return;}
  if(action==5){open_actions(NULL,NULL);return;}
  if(action==6)return;
  if(action==7){cancel_link();s_loading=false;if(s_body){s_page=0;s_loading=true;send_command(2,s_current_id,0,NULL);}else refresh_list();return;}
...
static void inbox(DictionaryIterator *iter, void *context) {
  ...
  if(kind==6) {
    ...
    Tuple *sort=dict_find(iter,MESSAGE_KEY_SORT),*tag=dict_find(iter,MESSAGE_KEY_TAG);
    if(sort){int value=sort->value->int32;s_sort=value>=0&&value<=3?value:0;}
    if(tag)snprintf(s_tag,sizeof(s_tag),"%s",tag->value->cstring);
    ...
    Tuple *index=dict_find(iter,MESSAGE_KEY_INDEX),*item=dict_find(iter,MESSAGE_KEY_ITEM_ID),*entry=dict_find(iter,MESSAGE_KEY_ENTRY_KIND),*checked=dict_find(iter,MESSAGE_KEY_CHECKED),*format=dict_find(iter,MESSAGE_KEY_FORMAT);int i=index?index->value->int32:-1;
    ...
    if(revision){snprintf(s_revision,sizeof(s_revision),"%s",revision->value->cstring);}s_snapshot[0]=0;s_before_search.snapshot[0]=0;set_status("Task saved");layer_mark_dirty(s_document_view);
...
#endif
    s_image=gbitmap_create_blank(GSize(w,h),GBitmapFormat8Bit);
#if !defined(PBL_PLATFORM_EMERY)
    while(!s_image&&w>32&&h>24){w=w*3/4;h=h*3/4;s_image=gbitmap_create_blank(GSize(w,h),GBitmapFormat8Bit);}
#endif
    s_image_received=0;s_image_pixel=0;s_image_bytes=total?total->value->int32:0;
    ...
    Tuple *bytes=dict_find(iter,MESSAGE_KEY_PIXELS),*offset=dict_find(iter,MESSAGE_KEY_INDEX);if(!bytes||!offset||offset->value->int32!=s_image_received||bytes->length%2){image_clear();set_status("Image interrupted · scroll away and back");return;}
...
static void main_load(Window *window) {
  ...
  window_attach_recognizer(window,swipe_recognizer_create(notesy_touch_main_swipe,NULL,SwipeDirection_Up|SwipeDirection_Down));
  window_attach_recognizer(window,tap_recognizer_create(notesy_touch_main_tap,NULL));
#endif
  s_menu=organik_menu_create(layer_get_bounds(window_get_root_layer(window)));
  menu_layer_set_callbacks(s_menu,NULL,(MenuLayerCallbacks){.get_num_rows=row_count,.get_cell_height=row_height,.draw_row=draw_row,.select_click=notesy_menu_selected,.selection_changed=browse_selection});
...
int main(void) {
  ...
  app_event_loop();if(s_browse_timer)app_timer_cancel(s_browse_timer);if(s_saved_refresh_timer)app_timer_cancel(s_saved_refresh_timer);stop_stitch();marquee_stop();clear_timeout();if(s_dictation)dictation_session_destroy(s_dictation);if(s_actions)window_destroy(s_actions);if(s_reader)window_destroy(s_reader);window_destroy(s_main);
#if defined(PBL_PLATFORM_EMERY)
  unload_custom_theme_font();
#endif
}
